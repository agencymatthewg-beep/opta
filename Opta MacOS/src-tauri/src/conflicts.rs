...
/// Flag to only log the opta_mcp warning once
static WARNED_OPTA_MCP: AtomicBool = AtomicBool::new(false);
...
pub async fn detect_conflicts() -> Result<ConflictSummary, String> {
    ...
    // Check if the response contains an error field - only warn once
    if stdout.contains("\"error\"") {
        let error: serde_json::Value = serde_json::from_str(&stdout)
            .map_err(|e| format!("JSON parse error: {}", e))?;
        if let Some(err_msg) = error.get("error").and_then(|v| v.as_str()) {
            // Only log the opta_mcp missing warning once to avoid spam
            if err_msg.contains("opta_mcp") {
                if !WARNED_OPTA_MCP.swap(true, Ordering::Relaxed) {
                    eprintln!("Note: opta_mcp module not found - some features disabled");
                }
            } else {
