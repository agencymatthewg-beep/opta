...
import { PROVIDERS, detectProvider, type ApiKeyProvider } from '@/lib/provider-detection';
import { type ApiKey, upsertApiKey, deleteApiKey, verifyApiKey } from '@/lib/supabase/key-actions';
...
export function KeysContent({ initialKeys }: KeysContentProps) {
  ...
  const handleSave = useCallback(
    async (provider: ApiKeyProvider, keyValue: string, label?: string) => {
      ...
      showToast(`${PROVIDERS.find((p) => p.id === provider)?.name} key saved.`, 'success');
      setSheetOpen(false);
