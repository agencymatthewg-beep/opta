    ...
    try {
        ...
        switch (action) {
            ...
            case "briefing": {
                ...
                const taskStats = tasksResult && "success" in tasksResult
                    ? tasksResult.data.stats
                    : { todayCount: 0, totalActive: 0 };

                const upcomingEvents = calendarResult && "success" in calendarResult
                ...
                const unreadCount = emailResult && "success" in emailResult
            ...
            case "quick_status": {
                ...
                const taskCount = tasksResult && "success" in tasksResult
                    ? tasksResult.data.stats.todayCount
                    : 0;

                const nextEvent = calendarResult && "success" in calendarResult && calendarResult.events.length > 0
                    ? calendarResult.events[0].summary
                    : null;

                const nextEventTime = calendarResult && "success" in calendarResult && calendarResult.events.length > 0
...
export async function GET(request: NextRequest) {
    try {
        ...
        const fakeRequest = {
            json: async () => ({ action: type === "briefing" ? "briefing" : "quick_status" }),
            url: request.url,
        } as NextRequest;
