...
export function SignInOverlay() {
  ...
  useEffect(() => {
    setRequiresAuthGate(window.location.protocol === 'https:');
  }, []);

  // Show overlay only in cloud/https mode when auth is required.
  const isVisible = requiresAuthGate && !isLoading && !user;
