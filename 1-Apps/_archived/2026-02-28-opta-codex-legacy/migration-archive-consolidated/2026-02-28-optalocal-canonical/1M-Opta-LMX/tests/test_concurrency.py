...
class TestPriorityLaneHardening:
    """High priority uses bounded privileged capacity, never unlimited bypass."""

    @pytest.mark.asyncio
    async def test_high_priority_is_bounded_by_privileged_lane(self) -> None:
        controller = ConcurrencyController(max_concurrent_requests=4)

        current = 0
        peak = 0
        lock = asyncio.Lock()
        queue_wait_ctx = contextvars.ContextVar("queue_wait", default=None)

        async def high_worker() -> None:
            nonlocal current, peak
            async with controller._acquire_request_slots(
                model_id="test/model-a",
                priority="high",
                client_id="trusted-admin",
                queue_wait_sec_ctx=queue_wait_ctx,
            ):
                async with lock:
                    current += 1
        ...
        await asyncio.gather(*(high_worker() for _ in range(3)))
        assert peak == 1

    @pytest.mark.asyncio
    async def test_normal_lane_preserves_fairness_capacity(self) -> None:
        controller = ConcurrencyController(max_concurrent_requests=4)
        ...
        await asyncio.gather(*(normal_worker() for _ in range(4)))
        assert peak == 3
