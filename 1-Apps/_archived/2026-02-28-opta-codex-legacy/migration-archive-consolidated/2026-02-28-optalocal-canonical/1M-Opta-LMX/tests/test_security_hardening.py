...
from opta_lmx.api.inference import _normalize_priority
...
class TestInferencePriorityAuth:
    """x-priority high only activates for trusted admin credentials."""

    def test_high_priority_without_admin_key_downgrades_to_normal(self) -> None:
        request = _make_request(admin_key="admin-secret")
        assert _normalize_priority(request, "high", None) == "normal"

    def test_high_priority_with_wrong_admin_key_downgrades_to_normal(self) -> None:
        request = _make_request(admin_key="admin-secret")
        assert _normalize_priority(request, "high", "wrong") == "normal"

    def test_high_priority_with_valid_admin_key_is_allowed(self) -> None:
        request = _make_request(admin_key="admin-secret")
        assert _normalize_priority(request, "high", "admin-secret") == "high"

    def test_non_high_priority_passes_without_admin_header(self) -> None:
        request = _make_request(admin_key="admin-secret")
        assert _normalize_priority(request, "interactive", None) == "interactive"
