...
async def _read_log_file(directory: Path, filename: str, suffixes: set[str]) -> str:
    """Read a specific log file async, guarding against path traversal."""
    ...
    # Add size guard for large files (10MB limit)
    try:
        stat = target.stat()
        ...
        if stat.st_size > max_size:
            raise HTTPException(
                status_code=413,
                detail=f"Log file too large ({stat.st_size / 1024 / 1024:.1f}MB > 10MB limit)",
            )
    except OSError as exc:
        logger.error("journal_log_stat_failed", extra={"path": str(target), "error": str(exc)})
