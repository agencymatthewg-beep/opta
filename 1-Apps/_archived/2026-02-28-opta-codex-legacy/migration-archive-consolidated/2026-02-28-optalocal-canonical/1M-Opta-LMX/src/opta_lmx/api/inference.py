...
def _normalize_priority(request: Request, x_priority: str | None, x_admin_key: str | None) -> str:
    allowed = {"low", "normal", "interactive", "background", "high"}
    priority = (x_priority or "normal").strip().lower()
    ...
    configured_admin_key: str | None = getattr(request.app.state, "admin_key", None)
    if configured_admin_key is None:
        return "normal"
    if x_admin_key is None:
        return "normal"
    if not secrets.compare_digest(x_admin_key, configured_admin_key):
        return "normal"
    return "high"
...
async def chat_completions(
    ...
    priority = _normalize_priority(request, x_priority, x_admin_key)
