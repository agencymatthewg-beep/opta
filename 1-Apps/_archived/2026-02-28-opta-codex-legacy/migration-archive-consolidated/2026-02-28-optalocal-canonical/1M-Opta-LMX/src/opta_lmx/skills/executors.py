...
class SkillExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        # Shared thread pool for skill execution to avoid per-call overhead
        self._thread_pool = ThreadPoolExecutor(
            max_workers=max(1, max_concurrent_calls), thread_name_prefix="skill_exec_"
        )
    ...
    def shutdown(self) -> None:
        """Clean up resources - must be called when executor is no longer needed."""
    ...
    def _call_with_timeout(
        ...
        *,
        ...
        if self._shutdown:
            raise RuntimeError("SkillExecutor has been shut down")

        future = self._thread_pool.submit(self._invoke_callable, function, arguments)
