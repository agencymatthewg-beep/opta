  ...
  return (
    <div className="fixed bottom-0 inset-x-0 z-40 h-7 glass-subtle border-t border-white/5 flex items-center px-4">
      ...
      {!isConnected ? (
        ...
        <>
          {/* Chip badge */}
          {identity?.hardware?.chip_name && (
            <span className="ml-2 font-mono text-[10px] text-neon-purple/70 border border-neon-purple/20 rounded px-1.5 py-0.5">
              {identity.hardware.chip_name.replace(/^Apple /, '')}
              {identity.hardware.memory_gb ? ` · ${identity.hardware.memory_gb}GB` : ''}
            </span>
          )}
