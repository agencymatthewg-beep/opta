...
- **D16**: Rate limiting deferred (slowapi added to pyproject.toml, not wired yet)
- **D17**: FAISS min-dim guard (dim < 32 → NumPy fallback) prevents ARM64 segfault
- **D18**: Metal cache maintenance enabled by default when metal_cache_limit_gb is set
...
## Resolved Deferred Issues (2026-02-19)

- ~~Rate limiting via slowapi — dependency not installed yet~~ → added to pyproject.toml [ratelimit] group
- ~~SSE integration tests (httpx-sse) — dev dependency not installed~~ → 6 SSE integration tests added
- ~~Metal cache maintenance loop — P3 stretch goal~~ → implemented as background task
- ~~FAISS segfault in unit tests~~ → fixed via min-dim guard (dim < 32 uses NumPy fallback)
- ~~BM25 ZeroDivisionError on single-char documents~~ → fixed with empty-vocabulary guard
...
- Rate limiting slowapi wiring into middleware — dependency installed, not yet configured
