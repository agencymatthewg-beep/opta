...
async def test_openclaw_perf_regression_gate() -> None:
    ...
    engine._generator._do_generate = fixed_cost_generate  # type: ignore[assignment]
...
async def test_benchmark_endpoint_overhead_under_50ms(
    ...
    # Force vllm-mlx backend so the mocked _create_engine path is used
    # instead of the real MLXLMBackend (which would attempt HuggingFace downloads).
    monkeypatch.setattr(
