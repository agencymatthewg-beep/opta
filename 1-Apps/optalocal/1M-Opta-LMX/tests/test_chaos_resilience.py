...
async def test_chaos_inference_timeout_does_not_leak_in_flight() -> None:
    ...
    engine._generator._do_generate = very_slow  # type: ignore[assignment]
...
async def test_chaos_loader_repeated_failures_eventually_quarantine() -> None:
    ...
    engine._lifecycle._create_engine = AsyncMock(return_value=(MagicMock(), {}))  # type: ignore[assignment]
    engine._lifecycle._run_load_canary = AsyncMock()  # type: ignore[assignment]
...
async def test_loader_chaos_500_crashes_zero_api_exits() -> None:
    """Plan Task 14 reliability gate: 500 forced loader failures, zero API process crashes.

    Mixes four failure modes: SIGKILL (signal 9), SIGSEGV (signal 6),
    timeout, and OOM to exercise all crash classification branches.
    """
    monitor = MemoryMonitor(max_percent=90)
    engine = InferenceEngine(memory_monitor=monitor, use_batching=False, warmup_on_load=False)
    engine._create_engine = AsyncMock(return_value=MagicMock())  # type: ignore[assignment]
    ...
    failure_sigkill = LoaderFailure(
        code=ErrorCodes.MODEL_LOADER_CRASHED,
        message="Child loader killed (SIGKILL)",
        signal=9,
    )
    failure_sigsegv = LoaderFailure(
        code=ErrorCodes.MODEL_LOADER_CRASHED,
        message="Child loader crashed (SIGSEGV)",
        signal=6,
    )
    failure_timeout = LoaderFailure(
        code=ErrorCodes.MODEL_LOAD_TIMEOUT,
        message="Child loader timed out after 120s",
    )
    failure_oom = LoaderFailure(
        code=ErrorCodes.MODEL_LOADER_CRASHED,
        message="Child loader OOM",
        signal=11,
        metadata={"oom": True},
    )

    failures = [failure_sigkill, failure_sigsegv, failure_timeout, failure_oom]
    attempts = 500
    sequence = [
        LoaderSupervisorOutcome(ok=False, failure=failures[idx % len(failures)])
        for idx in range(attempts)
    ]
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        for attempt in range(attempts):
            try:
                await engine.load_model(f"test/model-chaos-500-{attempt % 10}")
            except RuntimeError as exc:
                error_codes_seen.add(str(exc).split(":")[0])

    # All failure types produced deterministic error codes
    assert ErrorCodes.MODEL_LOADER_CRASHED in error_codes_seen
    assert ErrorCodes.MODEL_LOAD_TIMEOUT in error_codes_seen

    # All 10 model variants should be quarantined
    for variant in range(10):
        model_id = f"test/model-chaos-500-{variant}"
        assert_loader_failure_gate(engine=engine, model_id=model_id, attempts=50)

    # API process survived all 500 failures
    assert engine.in_flight_count == 0
