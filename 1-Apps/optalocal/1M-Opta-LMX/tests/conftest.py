...
def mock_engine() -> InferenceEngine:
    ...
    # The legacy wrapper on InferenceEngine returns just the engine mock.
    ...
    # Also patch the lifecycle manager directly (engine delegates to it)
    engine._lifecycle._create_engine = mock_create_lifecycle  # type: ignore[assignment]
...
def mock_model_manager(tmp_path: Path) -> ModelManager:
    ...
    async def _always_complete(model_id: str, revision: str | None = None) -> bool:
        return True

    manager.is_model_available = _always_available  # type: ignore[assignment]
    manager.is_local_snapshot_complete = _always_complete  # type: ignore[assignment]
    return manager
...
async def _make_test_client(
    ...
    *,
    ...
    async with AsyncClient(
        ...
        test_app.state.inference_api_key = None  # LAN mode — no inference auth
