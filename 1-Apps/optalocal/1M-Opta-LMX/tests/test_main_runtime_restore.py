...
@pytest.mark.asyncio
async def test_lifespan_invokes_runtime_directory_provisioning() -> None:
    """Lifespan should provision required runtime directories at startup."""
    config = LMXConfig(models={"auto_load": []})  # type: ignore[arg-type]
    app = create_app(config)
    ...
    with (
        patch("opta_lmx.main.RuntimeState", return_value=fake_runtime_state),
        patch("opta_lmx.main._ensure_runtime_directories") as ensure_dirs,
    ):
        async with app.router.lifespan_context(app):
            pass

    ensure_dirs.assert_called_once_with(config)
