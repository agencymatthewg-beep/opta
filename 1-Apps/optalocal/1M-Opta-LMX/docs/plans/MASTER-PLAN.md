...
### 5B. OpenClaw Integration [DEFERRED — separate repo `1I-OptaPlus`]
...
- 523 tests passing across 34 test files (3 skipped for optional deps)
...
- [x] Serves `/v1/models`
- [DEFERRED] Opta CLI `opta connect` + `opta do` works against it — Phase 5A, deferred to `1D-Opta-CLI-TS` repo
...
- [DEFERRED] OpenClaw bots work with zero config change — Phase 5B, deferred to `1I-OptaPlus` repo
- [x] GGUF fallback for models without MLX weights (gguf_backend.py, 14 tests, optional dep)
- [x] 523 tests passing across 34 test files
...
- [x] 523 tests across 34 test files
- [x] 35+ source modules in `src/opta_lmx/`
- [DEFERRED] Opta CLI fully migrated (no LM Studio references) — Phase 5A, deferred to `1D-Opta-CLI-TS` repo
- [DEFERRED] OpenClaw bots integrated — Phase 5B, deferred to `1I-OptaPlus` repo
- [~] Benchmarks published (vs LM Studio, vs Ollama) — internal benchmark suite implemented (`monitoring/benchmark.py`, `benchmarks/`), public comparison deferred
