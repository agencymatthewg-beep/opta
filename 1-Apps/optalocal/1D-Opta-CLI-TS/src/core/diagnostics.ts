...
export async function runConnectionDiagnostics(host: string, port: number): Promise<DiagnosticResult[]> {
  ...
  try {
    ...
    for (const altPort of altPorts) {
      ...
      } catch (err) { /* port not reachable */ console.error('Port check failed:', err); }
    }
  }
  ...
  if (results[0]?.status === 'ok') {
    ...
    } catch (err) { /* already reported */ if (process.env.OPTA_DEBUG) console.error('Connection check:', err); }
  }
