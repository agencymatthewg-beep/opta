...
/**
 * Read and parse a .gitignore file, converting patterns to fast-glob compatible ignore entries.
 *
 * - Blank lines and comments (lines starting with #) are skipped.
 * - Rooted patterns (starting with /) are made relative to cwd.
 * - Simple name patterns (no slashes) are prefixed with ** / to match anywhere.
 * - Negation patterns (starting with !) are excluded — fast-glob does not support them in ignore.
 */
export function loadGitignorePatterns(cwd: string): string[] {
  ...
  return content
    ...
    .map((pattern) => {
      // Strip trailing slashes — fast-glob treats directory patterns the same
      const cleaned = pattern.replace(/\/+$/, '');

      if (cleaned.startsWith('/')) {
        // Rooted pattern — relative to cwd, match directory contents too
        const rooted = cleaned.slice(1);
        return `${rooted}/**`;
      }
      if (!cleaned.includes('/')) {
        // Simple name — match anywhere in tree
        return `**/${cleaned}/**`;
      }
      return cleaned;
    });
...
export async function scanExports(cwd: string): Promise<ExportMap> {
  ...
  // Build ignore patterns from IGNORED_DIRS + .gitignore
  const hardcodedIgnore = IGNORED_DIRS.map((dir) => `**/${dir}/**`);
  const gitignoreIgnore = loadGitignorePatterns(cwd);
  const ignore = [...hardcodedIgnore, ...gitignoreIgnore];
