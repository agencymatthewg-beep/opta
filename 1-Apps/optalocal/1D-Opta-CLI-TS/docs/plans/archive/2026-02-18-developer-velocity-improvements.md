...
> **Status (verified 2026-02-27):** 16/18 tasks DONE, 1 PARTIAL, 1 NOT DONE.
>
> | Task | Status | Notes |
> |------|--------|-------|
> | 1. utils/errors.ts | DONE | Enhanced implementation with recursive property extraction |
> | 2. Token consolidation | DONE | utils/tokens.ts with estimateTokens/formatTokens |
> | 3. Remove stubs.ts | DONE | File deleted |
> | 4. Dynamic version in server.ts | DONE | Imports VERSION from core/version.js |
> | 5. SearXNG URL fix | DONE | Config-driven, no hardcoded fallback |
> | 6. safeParseJson utility | DONE | utils/json.ts with generic overload |
> | 7. Shared ignore patterns | DONE | utils/ignore.ts with DEFAULT_IGNORE_DIRS |
> | 8. Config store singleton | DONE | getConfigStore() with module-level cache |
> | 9. Hoist dynamic imports | DONE | Imports before while(true) loop |
> | 10. Vitest coverage thresholds | DONE | 30/20/25/30 (slightly lower than plan's 35/25/30/35) |
> | 11. Named magic constants | DONE | 6 constants in agent-setup.ts |
> | 12. Shared model scan service | **NOT DONE** | No src/services/ directory; models.ts and lmx.ts each have own scan |
> | 13. SubAgentResult builder | DONE | Closure approach in subagent.ts |
> | 14. Replace process.exit | **PARTIAL** | agent.ts clean; chat.ts:228 still has process.exit(1) |
> | 15. Extract agent-streaming.ts | DONE | |
> | 16. Extract agent-permissions.ts | DONE | |
> | 17. Extract agent-execution.ts | DONE | |
> | 18. Extract agent-setup.ts | DONE | |
