# Auto Task: Plan mode (read-only planning)

## Why now
Competitive gap score is **3** (HIGH). Multiple competitors now ship this capability.

## Objective
Implement **Plan mode (read-only planning)** in Opta CLI with production-safe defaults and no regression to current behavior.
