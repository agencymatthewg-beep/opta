...
describe('PermissionPrompt', () => {
  ...
  it('renders browser spawn context when prompting to open browser session', () => {
    const onDecision = vi.fn();
    const { lastFrame, unmount } = render(
      <PermissionPrompt
    ...
    const frame = lastFrame()!;
    expect(frame).toContain('browser_open');
    expect(frame).toContain('Spawn Opta Browser session before continuing.');
    expect(frame).toContain('requested by: browser_navigate');
    expect(frame).toContain('runtime scan: 0 active sessions');
    unmount();
  });
});
