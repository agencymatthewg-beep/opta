...
import { ScrollView, computeEstimationWidth, estimateChildLines, estimateTextLines } from '../../src/tui/ScrollView.js';
...
describe('estimateTextLines', () => {
  ...
  it('should handle very narrow width', () => {
    expect(estimateTextLines('hello world', 5)).toBe(3); // 11 chars / 5 = 3
  });

  it('should clamp estimation width to at least 1 column', () => {
    expect(computeEstimationWidth(0)).toBe(1);
    expect(computeEstimationWidth(1)).toBe(1);
    expect(computeEstimationWidth(6)).toBe(5);
  });

  it('should ignore ANSI escape sequences in width estimation', () => {
    expect(estimateTextLines('\u001B[32mhello\u001B[0m', 80)).toBe(1);
  });

  it('should handle wide glyph width', () => {
    // "你" is treated as width 2, so 6 glyphs = 12 columns => 2 rows at width 10
    expect(estimateTextLines('你你你你你你', 10)).toBe(2);
  });
});

describe('estimateChildLines', () => {
  it('should estimate line count for custom components that expose a text prop', () => {
    const CustomText = ({ text }: { text: string }) => <Text>{text}</Text>;
    const el = <CustomText text={'line1\nline2\nline3'} />;
    expect(estimateChildLines(el, 80)).toBe(3);
  });

  it('prefers estimatedText when provided by markdown wrappers', () => {
    const MarkdownLike = ({ text, estimatedText, width }: { text: string; estimatedText: string; width: number }) => (
      <Text>{text}</Text>
    );
    const el = (
      <MarkdownLike
        text={'summary'}
        estimatedText={'row-1 details\nrow-2 details\nrow-3 details'}
        width={16}
      />
    );
    // estimatedText should drive line counting instead of the short raw text prop.
    expect(estimateChildLines(el, 16)).toBe(3);
  });
  ...
  it('should use max line height for row layout children', () => {
    const long = 'x'.repeat(160); // wraps to 2 lines at width 80
    const el = (
...
describe('ScrollView', () => {
  ...
  it('should not render a trailing item that cannot fully fit in viewport', () => {
    const { lastFrame } = render(
      <ScrollView height={5} autoScroll={false}>
    ...
    expect(frame).not.toContain('Second card');
  });
});
