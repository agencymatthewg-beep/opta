...
describe('Sidebar', () => {
  ...
  it('shows live thinking pane content in sidebar', () => {
    const { lastFrame } = render(
      <Sidebar
        model="test"
        sessionId="abc"
        ...
        mode="normal"
        elapsed={0}
        liveThinkingText="Validating layout stability and spacing."
        liveThinkingTokens={12}
        thinkingExpanded={true}
        thinkingActive={true}
      />
    );
    const frame = lastFrame();
    expect(frame).toContain('Live Thinking');
    expect(frame).toContain('State');
    expect(frame).toContain('Live');
    expect(frame).toContain('Tokens');
    expect(frame).toContain('Validating layout');
  });
});
