...
- [x] Assistant messages in TUI display styled markdown (headings, bold, italic, links, lists)
- [x] Code blocks render with syntax-highlighted colors (amber/yellow via marked-terminal)
- [x] Streaming messages debounce re-renders (no flicker or parsing on every token)
- [x] Completed messages are cached (no redundant re-parsing)
- [x] Terminal width is respected (no horizontal overflow)
- [x] User messages remain plain text (no markdown processing)
- [x] All existing tests pass (zero regressions)
- [x] New tests cover: basic rendering, streaming, caching, width
