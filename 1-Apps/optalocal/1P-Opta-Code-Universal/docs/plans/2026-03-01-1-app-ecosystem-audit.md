...
|---|---|---|---|
| `1P-Opta-Code-Universal` | React + Vite | Yes (`src-tauri`) | Only active universal-capable app shell |
...
- `1P-Opta-Code-Universal` (formerly `1P-Opta-Code-Desktop`) and `1L-Opta-Local/web` both implement operator-facing daemon/session UX concerns.
...
`1P-Opta-Code-Universal` is validated as the Golden Repo because:
...
### Deprecation execution update
- `1L-Opta-Local` was archived and then permanently deleted after approval.
- Safety backup retained at `/tmp/opta-1L-Opta-Local-backup-20260301.tgz`.
