...
/**
 * POST /api/oauth/copilot/device/poll
 *
 * Polls GitHub for Device Flow completion. Reads device_code from the
 * HttpOnly cookie set by /device/start — it is never sent to or from clients.
 *
 ...
 * then clears the device_code cookie.
 */
export async function POST() {
  ...
  const deviceCode = cookieStore.get(COPILOT_COOKIE)?.value;
  if (!deviceCode) {
  ...
  if (ghData.access_token) {
      ...
      .upsert(
        {
          ...
          token_scope: ghData.scope ?? 'copilot',
    ...
    response.cookies.delete(COPILOT_COOKIE);
    return response;
  }
