...
1. ~~Manifest schema breadth~~ → **Being implemented** (skill_id, output_schema, model_preferences, roots)
2. ~~Budget-aware dispatch~~ → **Being implemented** (token_budget, cost_budget_usd, BudgetExhaustedError)
3. External broker-backed workers (Celery/Redis/RabbitMQ) → **Still deferred** (in-process worker plane is sufficient for current scale)
4. ~~MCP surface depth~~ → **Being implemented** (prompts/list, prompts/get, resources/list, capabilities)
5. ~~Trace/audit depth~~ → **Being implemented** (AuditTrail, checkpoint_pointer, extract_trace_id)

---

## Success Criteria

1. Run 5+ concurrent agents with bounded latency and no starvation.
2. Skills are first-class assets with manifest validation, versioning, and permissions.
3. Skill discovery/execution APIs follow MCP-aligned primitives.
4. Agent runs are resumable, traceable, and auditable.
5. Orchestration scales from single-process to distributed workers without API redesign.
...
## Phase A — Foundations (Schema + Config + Skeleton)
...
## Phase B — In-Process Multi-Agent Runtime
...
## Phase C — Skills-Native APIs and MCP Alignment
...
## Phase D — Persistence, Recovery, and Traceability
...
## Phase E — Distributed Worker Plane (Optional but Recommended)
...
## Phase F — Hardening and Competitive Benchmarks
...
1. All phases implemented or explicitly deferred with recorded rationale.
2. New APIs documented in `docs/plans/API-SPEC.md`.
3. CI covers new agent/skills test suites.
4. Operational runbook updated in `docs/WORKFLOWS.md`.
5. Demo scenario proves:
   - parallel multi-agent run
   - skill discovery/execution
   - approval-gated high-risk skill
   - trace-linked observability across full run.
