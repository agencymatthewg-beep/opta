...
_DEFAULT_MIN_FREE_DISK_BYTES = 5 * (1024**3)
_DEFAULT_DOWNLOAD_ESTIMATE_MARGIN_RATIO = 1.10
...
class ModelManager:
    ...
    def __init__(
        ...
        *,
        download_free_space_floor_gb: float = 0.0,
        download_estimate_margin_ratio: float = _DEFAULT_DOWNLOAD_ESTIMATE_MARGIN_RATIO,
        ...
        self._download_free_space_floor_bytes = max(
            int(download_free_space_floor_gb * (1024**3)),
            0,
        )
        self._download_estimate_margin_ratio = max(download_estimate_margin_ratio, 1.0)
    ...
    async def start_download(
        ...
        if self._active_download_count() >= self._max_concurrent_downloads:
            raise RuntimeError(
                ...
                f"{self._active_download_count()}/{self._max_concurrent_downloads} active"
            )
        ...
        try:
            ...
            if disk.free < self._download_free_space_floor_bytes:
                raise OSError(
                    f"Insufficient disk space: {disk.free / (1024**3):.1f} GB free, "
                    f"need at least {self._download_free_space_floor_bytes / (1024**3):.1f} GB free before download"
                )
        except OSError:
            raise
        ...
        try:
            if estimated_size > 0:
                cache_dir = self._cache_dir()
                disk = shutil.disk_usage(cache_dir if cache_dir.exists() else Path.home())
                required = int(estimated_size * self._download_estimate_margin_ratio)
    ...
    async def sweep_incomplete_artifacts(self, min_age_sec: float = 1800.0) -> dict[str, Any]:
        """Delete stale incomplete HF snapshot revisions from cache.

        A revision is considered incomplete if required safetensor shard files are missing
        according to `model.safetensors.index.json`.
        """
        now = time.time()
        results: dict[str, Any] = {
            "checked_revisions": 0,
            "incomplete_revisions": 0,
            "deleted_revisions": 0,
            "freed_bytes": 0,
            "errors": [],
        }
        ...
        try:
            ...
            logger.warning("incomplete_sweep_cache_scan_failed", extra={"error": str(e)})
            results["errors"].append(str(e))
            return results

        stale_commit_hashes: list[str] = []
        for repo in cache_info.repos:
            for rev in getattr(repo, "revisions", []):
                results["checked_revisions"] += 1
                snapshot_path = self._revision_snapshot_path(rev)
                if snapshot_path is None:
                    continue

                last_modified = getattr(rev, "last_modified", None)
                try:
                    modified_at = float(last_modified.timestamp()) if last_modified is not None else 0.0
                except Exception:
                    modified_at = 0.0
                if modified_at and (now - modified_at) < min_age_sec:
                    continue
                ...
                results["incomplete_revisions"] += 1
                commit_hash = getattr(rev, "commit_hash", None)
                if isinstance(commit_hash, str) and commit_hash:
                    stale_commit_hashes.append(commit_hash)
                    logger.info(
                        "incomplete_snapshot_detected",
                        extra={
                            "repo_id": repo.repo_id,
        ...
        try:
            delete_strategy = cache_info.delete_revisions(*stale_commit_hashes)
            expected_freed = int(getattr(delete_strategy, "expected_freed_size", 0) or 0)
            await asyncio.to_thread(delete_strategy.execute)
            self._invalidate_available_cache()
            results["deleted_revisions"] = len(stale_commit_hashes)
            results["freed_bytes"] = expected_freed
        except Exception as e:
            logger.warning("incomplete_sweep_delete_failed", extra={"error": str(e)})
            results["errors"].append(str(e))
    ...
    def _active_download_count(self) -> int:
        return sum(
            ...
            if task.status == "downloading" and task.task is not None and not task.task.done()
        )
