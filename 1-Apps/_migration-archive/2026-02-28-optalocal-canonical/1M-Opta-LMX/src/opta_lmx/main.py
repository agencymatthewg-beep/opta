...
from opta_lmx.api.journal_logs import router as journal_logs_router
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    model_manager = ModelManager(
        ...
        event_bus=event_bus,
        download_free_space_floor_gb=config.models.download_free_space_floor_gb,
        download_estimate_margin_ratio=config.models.download_estimate_margin_ratio,
    ...
    if config.journaling.enabled:
        try:
            ...
            # Prune stale log files based on retention policy
            try:
                pruned = journal_manager.prune_old_logs()
                if pruned:
                    logger.info("journaling_pruned_on_startup", extra={"deleted": pruned})
            except Exception:
                logger.warning("journaling_prune_failed_on_startup", exc_info=True)
    ...
    # Start incomplete artifact sweeper background task if enabled
    incomplete_sweeper_task: asyncio.Task[None] | None = None
    if config.models.incomplete_artifact_sweeper_enabled:

        # Run one sweep immediately at startup to reclaim stale partial artifacts fast.
        try:
            initial_sweep = await model_manager.sweep_incomplete_artifacts(
                min_age_sec=config.models.incomplete_artifact_min_age_sec,
            )
            if initial_sweep.get("deleted_revisions", 0) > 0:
                logger.info("incomplete_artifact_sweep_completed", extra=initial_sweep)
        except Exception:
            logger.warning("incomplete_artifact_sweeper_initial_run_error")

        async def _incomplete_artifact_sweeper_loop() -> None:
            while True:
                await asyncio.sleep(config.models.incomplete_artifact_sweeper_interval_sec)
                try:
                    sweep = await model_manager.sweep_incomplete_artifacts(
                        min_age_sec=config.models.incomplete_artifact_min_age_sec,
                    )
                    if sweep.get("deleted_revisions", 0) > 0:
                        logger.info("incomplete_artifact_sweep_completed", extra=sweep)
                except asyncio.CancelledError:
                    raise
                except Exception:
                    logger.warning("incomplete_artifact_sweeper_loop_error")

        incomplete_sweeper_task = asyncio.create_task(
            _incomplete_artifact_sweeper_loop(),
            name="incomplete-artifact-sweeper-loop",
        )
        logger.info("incomplete_artifact_sweeper_started")
    ...
    if incomplete_sweeper_task is not None:
        incomplete_sweeper_task.cancel()
        with contextlib.suppress(asyncio.CancelledError):
            await incomplete_sweeper_task
    ...
    # Cleanup: shutdown skill executor thread pool
...
def create_app(config: LMXConfig | None = None) -> FastAPI:
    ...
    app.include_router(journal_logs_router)
