...
def test_parse_json_output_none_format():
    text, parsed, valid, err = parse_json_output("hello", None)
    assert text == "hello"
...
def test_parse_json_output_json_object():
    text, parsed, valid, err = parse_json_output('{"x": 1}', {"type": "json_object"})
    assert parsed == {"x": 1}
    assert valid is True
    assert err is None
...
def test_parse_json_output_invalid():
    text, parsed, valid, err = parse_json_output("no json", {"type": "json_object"})
    assert parsed is None
...
def test_parse_json_output_json_schema_valid():
    ...
    text, parsed, valid, err = parse_json_output('{"name": "Alice"}', schema)
