...
### OPIS Integration
- [ ] `buildSystemPrompt()` includes OPIS summary when scaffold exists
- [ ] `read_project_docs` tool returns correct file contents
- [ ] Fallback to `.opta/memory.md` when no OPIS scaffold
- [ ] Non-OPIS projects get one-time suggestion to run `opta init`

### Git Integration
- [ ] Checkpoint patches created after each edit_file/write_file
- [ ] `/undo` reverses the correct patch
- [ ] Task completion creates one clean commit
- [ ] `--no-commit` flag disables auto-commit
- [ ] Dirty working tree warning at session start
- [ ] No git features in non-git directories (graceful skip)

### MCP Integration
- [ ] stdio transport connects and lists tools
- [ ] HTTP transport connects and lists tools
- [ ] MCP tools appear in model's tool list
- [ ] MCP tool calls route to correct server
- [ ] Failed MCP server doesn't block session
- [ ] `opta mcp list` shows configured servers
- [ ] Tool count warning at >20 tools

### Repo Awareness
- [ ] Export map generated in <500ms
- [ ] Respects .gitignore
- [ ] Handles TypeScript, Python, Swift
- [ ] Truncation works for large repos (>100 files)
- [ ] Map included in system prompt

### `opta init`
- [ ] Mode auto-detection works for greenfield and brownfield
- [ ] 10 questions asked via inquirer prompts
- [ ] Model generates valid OPIS scaffold files
- [ ] MCP suggestions parsed from KNOWLEDGE.md
- [ ] Existing OPIS docs not overwritten without --force
