...
4. **`docs/DECISIONS.md`**
   - Why OpenAI function-call format?
   - Why lazy loading?
   - Why single-threaded agent loop?
   - Why direct API (no daemon)?
...
6. **`CLAUDE.md`** (this repo root) — REPLACED with OPIS-quality version
...
**Time:** ~30 minutes | **Why:** You need to know which file to edit and how it connects.
...
9. **`docs/ROADMAP.md`**
   - V0 (current BETA state)
   - V1 (3 capabilities, ready to implement)
   - V2+ (plugins, MCP, cloud fallback, swarms)
   - Why things were deferred
