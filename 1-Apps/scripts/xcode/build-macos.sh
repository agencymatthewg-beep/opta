...
Build a macOS scheme with deterministic package resolution.

Options:
  --target <name>         Known preset target (default: native-macos)
...
destination="${MACOS_DESTINATION:-platform=macOS}"
...
if [[ -z "${project_arg}" || -z "${scheme_arg}" ]]; then
  ...
  if [[ "${XCODE_TARGET_PLATFORM}" != "macos" ]]; then
    xcode_die "Target '${target}' is not a macOS target"
  fi
...
xcode_prepare_output_paths "build-macos" "${scheme_arg}" "${configuration}"

xcode_info "Building macOS scheme '${scheme_arg}'"
