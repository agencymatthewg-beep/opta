...
private enum MenuConstants {
    ...
    static let allWebsiteURLs = [websiteURL, usageURL, aiCompareURL, lifeManagerURL]
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func addWebsiteSection(to menu: NSMenu) {
        ...
        let usageItem = NSMenuItem(
            title: "Open Opta Usage",
            action: #selector(openUsageDashboard),
            keyEquivalent: "u"
        )
        usageItem.keyEquivalentModifierMask = [.command, .shift]
        usageItem.target = self
        usageItem.image = NSImage(systemSymbolName: MenuConstants.Icons.usage, accessibilityDescription: "Usage Dashboard")
    ...
    @objc private func openUsageDashboard() {
        guard let url = URL(string: MenuConstants.usageURL) else { return }
        NSWorkspace.shared.open(url)
    }
