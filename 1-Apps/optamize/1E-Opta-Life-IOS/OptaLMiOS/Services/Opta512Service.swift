...
class Opta512Service: ObservableObject {
    ...
    private enum Opta512DataFile: String {
        case schedule = "SCHEDULE.md"
        case priorities = "PRIORITIES.md"

        var environmentOverrideKey: String {
            switch self {
            case .schedule:
                return "OPTA512_SCHEDULE_PATH"
            case .priorities:
                return "OPTA512_PRIORITIES_PATH"
            }
        }
        ...
        var fallbackContent: String {
            ...
            case .priorities:
                ...
                | Project | Milestone | Deadline | Status |
                | --- | --- | --- | --- |
                """
            }
        }
    ...
    private let opta512DirectoryName = "Opta512"
    ...
    private func readSchedule() async throws -> Opta512Schedule {
        let content = loadMarkdownContent(for: .schedule)
        return try parseSchedule(content)
    }
    
    private func readPriorities() async throws -> Opta512Priorities {
        let content = loadMarkdownContent(for: .priorities)
        return try parsePriorities(content)
    }

    private func loadMarkdownContent(for file: Opta512DataFile) -> String {
        let fm = FileManager.default

        if
            let overridePath = ProcessInfo.processInfo.environment[file.environmentOverrideKey],
            !overridePath.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        {
            do {
                let content = try String(contentsOfFile: overridePath, encoding: .utf8)
                print("[Opta512Service] Loaded \(file.rawValue) from \(file.environmentOverrideKey)")
                return content
            } catch {
                print("[Opta512Service] Failed reading \(file.environmentOverrideKey): \(error)")
            }
        }

        for url in sandboxCandidateURLs(for: file, fileManager: fm) {
            guard fm.fileExists(atPath: url.path) else { continue }

            do {
                let content = try String(contentsOf: url, encoding: .utf8)
                print("[Opta512Service] Loaded \(file.rawValue) from \(url.path)")
                return content
            } catch {
                print("[Opta512Service] Failed reading \(url.path): \(error)")
            }
        }

        let baseName = file.rawValue.replacingOccurrences(of: ".md", with: "")
        if
            let url = Bundle.main.url(forResource: baseName, withExtension: "md"),
            let content = try? String(contentsOf: url, encoding: .utf8)
        {
            print("[Opta512Service] Loaded \(file.rawValue) from app bundle")
            return content
        }

        print("[Opta512Service] Using embedded fallback for \(file.rawValue)")
        return file.fallbackContent
    }

    private func sandboxCandidateURLs(for file: Opta512DataFile, fileManager: FileManager) -> [URL] {
        var urls: [URL] = []

        if let documentsURL = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first {
            urls.append(documentsURL.appendingPathComponent(opta512DirectoryName, isDirectory: true).appendingPathComponent(file.rawValue))
            urls.append(documentsURL.appendingPathComponent(file.rawValue))
        }

        if let supportURL = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
            urls.append(supportURL.appendingPathComponent(opta512DirectoryName, isDirectory: true).appendingPathComponent(file.rawValue))
            urls.append(supportURL.appendingPathComponent(file.rawValue))
        }
