...
function mapTask(task: TodoistTask, projectMap: Map<string, string>): DashboardTask {
    return {
        id: task.id,
...
function buildDashboardResponse(data: {
    ...
    const projectMap = new Map<string, string>(data.projects.map((p) => [p.id, p.name]));
    ...
    return {
        ...
        data: {
            ...
            upcomingTasks: data.upcomingTasks.map((task) => mapTask(task, projectMap)),
    ...
    try {
        ...
        switch (view) {
            case "today": {
                ...
                return NextResponse.json({ success: true, source: "todoist", data: result.data });
            }
