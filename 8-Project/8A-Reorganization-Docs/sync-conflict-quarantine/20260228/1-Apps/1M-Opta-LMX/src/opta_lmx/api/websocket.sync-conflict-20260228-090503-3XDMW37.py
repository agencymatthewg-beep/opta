...
def _extract_ws_credentials(websocket: WebSocket) -> tuple[str | None, str | None]:
    auth_header = websocket.headers.get("authorization")
    token = _extract_bearer_token(auth_header)
    api_key = websocket.headers.get("x-api-key")

    if websocket.scope.get("query_string"):
        query = parse_qs(websocket.scope["query_string"].decode("utf-8"))
        token = token or (query.get("token", [None])[0] or query.get("api_key", [None])[0])
        api_key = api_key or query.get("x_api_key", [None])[0]

    return token, api_key


def _verify_ws_inference_auth(websocket: WebSocket) -> bool:
    inference_api_key: str | None = getattr(websocket.app.state, "inference_api_key", None)
    if inference_api_key is None:
        return True

    token, api_key = _extract_ws_credentials(websocket)
    candidate = token or api_key
    return bool(candidate and secrets.compare_digest(candidate, inference_api_key))
...
async def websocket_chat(websocket: WebSocket) -> None:
    ...
    """
    if not _verify_ws_inference_auth(websocket):
