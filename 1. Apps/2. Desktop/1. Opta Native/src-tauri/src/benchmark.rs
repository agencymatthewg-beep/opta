...
try:
    ...
    import time
    print(json.dumps({"benchmarkId": "unavailable", "gameId": sys.argv[1] if len(sys.argv) > 1 else "", "phase": sys.argv[3] if len(sys.argv) > 3 else "", "startedAt": time.time(), "status": "unavailable"}))
